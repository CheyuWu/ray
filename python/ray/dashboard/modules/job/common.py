...
class JobRetryCondition(str, Enum):
    ...
    #: Retry when the driver appears to have been OOM-killed. This is a
    #: best-effort heuristic that matches exit code 137 (128 + SIGKILL) and is
...
class RetryPolicy:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.backoff_multiplier, (int, float)) or isinstance(
            ...
            raise TypeError(
                "backoff_multiplier must be a number, got "
                f"{type(self.backoff_multiplier)}"
            )
        ...
        if self.no_retry_on_exit_codes is not None:
            if not isinstance(self.no_retry_on_exit_codes, list):
                raise TypeError(
                    ...
                    f"got {type(self.no_retry_on_exit_codes)}"
                )
            for code in self.no_retry_on_exit_codes:
                if not isinstance(code, int) or isinstance(code, bool):
                    raise TypeError(
                        "no_retry_on_exit_codes must be a list of ints, "
                        f"got element {code!r}"
                    )
...
class JobSubmitRequest:
    ...
    def _normalize_and_validate_retry_policy(
        ...
        if not isinstance(retry_policy, dict):
            raise TypeError(
                f"retry_policy must be a dict, got {type(retry_policy)}"
            )
        flat = dict(retry_policy)
        backoff = flat.pop("backoff", None)
        if backoff is not None:
            if not isinstance(backoff, dict):
                raise TypeError(
                    f"retry_policy.backoff must be a dict, got {type(backoff)}"
                )
            if "initial_delay_seconds" in backoff:
                flat["backoff_initial_delay_seconds"] = backoff["initial_delay_seconds"]
            if "max_delay_seconds" in backoff:
                flat["backoff_max_delay_seconds"] = backoff["max_delay_seconds"]
            if "multiplier" in backoff:
                flat["backoff_multiplier"] = backoff["multiplier"]
        # Construct RetryPolicy to validate; reject unknown keys explicitly.
        try:
            RetryPolicy(**flat)
        except TypeError as e:
            raise TypeError(f"Invalid retry_policy: {e}")
        return flat
